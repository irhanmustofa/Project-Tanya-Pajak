...
export function TeamSwitcher({ teams = [] }) {
  ...
  const getInitialTeam = () => {
    if (savedClientId && teams.length > 0) {
      const savedClient = teams.find((team) => team._id === savedClientId);
  ...
  const handleTeamSelect = (client) => {
    ...
    useLocalStorage.set("clientId", client._id);
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <div className="flex aspect-square size-8 items-center justify-center rounded-lg  text-sidebar-primary-foreground">
                {activeTeam.logo ? (
                  <img
                    src={base_url + `/public` + activeTeam.logo}
                    alt={activeTeam.name}
              ...
              filteredTeams.map((client, index) => (
                ...
                >
                  <div className="flex size-6 items-center justify-center rounded-sm border">
                    {client.logo ? (
                      <img
                        src={base_url + `/public` + client.logo}
                        alt={client.name}
