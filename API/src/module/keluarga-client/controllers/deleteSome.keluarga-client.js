...
export default async function deleteSomeKeluarga(req) {
  const newData = [];
  const ids = req.body;
  ...
  try {
    ...
    if (singleData.length < 1) {
      return badRequest({
        message: "Delete some Keluarga client failed! data not found",
      });
    }
    ...
    return await wrapper.update(clientId, { data_keluarga: newData });
  } catch (err) {
    console.log("delete Some Keluarga client error:", err);
    return error({
      message:
