...
import {
  useJurnal,
  useJurnalDispatch,
} from "../jurnal-components/JurnalProvider";
import { LucideUserPlus2 } from "lucide-react";
import { jurnalUpdate, jurnalAll } from "../jurnal-components/JurnalService";
import { useDialog, useDialogDispatch } from "@/dialogs/DialogProvider";
import { InputHorizontal } from "@/components/custom/input-custom";
import { useValidateInput } from "@/hooks/use-validate-input";
import { Textarea } from "@/components/ui/textarea";

export default function JurnalUpdateForm({ id, onClose }) {
  ...
  const { junalAction, jurnalState, jurnalCoa, params } = useJurnal();
  ...
  useEffect(() => {
    const data = jurnalState.data.find((item) => item._id === id);
    if (data) {
      setInput({ ...data });
      setIsOpen(true);
    }
  ...
  const inputHandler = (event) => {
    ...
    startTransition(async () => {
      ...
      await jurnalUpdate(id, formData).then((response) => {
      ...
      {isOpen && (
        <Dialog onOpenChange={handleCloseDialog} open={isOpen}>
          ...
          <DialogContent className="sm:max-w-5xl">
            <DialogTitle>Update Jurnal</DialogTitle>
            <DialogDescription>
              Update transaksi jurnal yang ada.
            </DialogDescription>
