...
import { pajakMasukanAll, pajakMasukanEndpoint } from "./PajakMasukanService";
import { useDialog, useDialogDispatch } from "@/dialogs/DialogProvider";
import {
  usePajakMasukan,
  usePajakMasukanDispatch,
} from "./PajakMasukanProvider";
...
export default function PajakMasukanAction({ row }) {
  const dispatch = useDialogDispatch();
  const pajakMasukanDispatch = usePajakMasukanDispatch();
  const { dialogState, dialogAction, DialogDelete } = useDialog();
  const { pajakMasukanAction } = usePajakMasukan();
  ...
  const handleDelete = (id) => {
    dispatch({
      ...
      payload: {
        ...
        status: "warning",
        url: pajakMasukanEndpoint.delete(id),
      },
    });
  };
  ...
  const handleOnCloseDelete = async (success) => {
    ...
    await pajakMasukanAll().then((res) => {
      if (res.success) {
        pajakMasukanDispatch({
          type: pajakMasukanAction.SUCCESS,
          payload: res.data,
        });
      } else {
        pajakMasukanDispatch({
          type: pajakMasukanAction.ERROR,
          payload: res.message,
        });
      }
    });
