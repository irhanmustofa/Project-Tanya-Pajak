...
export default function JurnalUmumAddForm({ onClose }) {
  ...
  const { jurnalUmumAction, coas } = useJurnalUmum();
  ...
  const handleSubmit = (event) => {
    ...
    startTransition(async () => {
      ...
      await jurnalUmumCreate(formData).then((response) => {
        if (response.success) {
          dialogDispatch({
    ...
    <div className="relative">
      ...
      <Dialog open={isOpen} onOpenChange={handleOnClose}>
        <DialogContent className="sm:max-w-[600px]">
          ...
          <DialogDescription>Tambah data secara manual.</DialogDescription>
